...
static int is_sgx_supported(int cpunum)
{
	...
	return ((cpuid_regs.ebx & SGX_SUPPORTED) && (msr.io32.lo & PRMRR_SUPPORTED));
}
...
static int get_number_of_cores(void)
{
    ...
    sysctlbyname("machdep.cpu.core_count", &ret, &sz, NULL, 0);
    return ret;
}
#endif
 ...
 */
	...
	for (i = 0; i < cpu->num_global_msrs; i++) {
		...
		printf(" MSR 0x%08X = 0x%08X:0x%08X (%s)\n",
		       cpu->global_msrs[i].number, msr.io32.hi, msr.io32.lo,
	...
	for (core = 0; core < core_num; core++) {
		...
		for (i = 0; i < cpu->num_per_core_msrs; i++) {
			...
			printf(" MSR 0x%08X = 0x%08X:0x%08X (%s)\n",
			       cpu->per_core_msrs[i].number, msr.io32.hi, msr.io32.lo,
